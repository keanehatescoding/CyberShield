...
class EnsureUserProfileUseCaseTest {
    ...
    @Test
    fun `non-missing-profile error returns NotApplicable with original message`() = runTest {
    ...
    @Test
    fun `missing profile with no session returns NotApplicable and does not attempt repair`() = runTest {
    ...
    @Test
    fun `missing profile with session attempts repair using uid and email`() = runTest {
    ...
    @Test
    fun `repair failure returns RepairFailed`() = runTest {
    ...
    @Test
    fun `second call without a success in between returns AlreadyAttempted and does not retry`() = runTest {
        fakeUserRepository.createUserProfileIfNotExistsResult = Result.Success(Unit)
    ...
    @Test
    fun `onProfileLoadedSuccessfully resets the attempted flag, allowing repair again`() = runTest {
        fakeUserRepository.createUserProfileIfNotExistsResult = Result.Success(Unit)
        useCase(Exception("Profile not found"), session)
    ...
    @Test
    fun `AlreadyAttempted after a failed repair still surfaces the underlying message`() = runTest {
        fakeUserRepository.createUserProfileIfNotExistsResult =
